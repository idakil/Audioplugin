...
{
    double phaseIncrement = frequency / samplerate * juce::MathConstants<double>::twoPi;

    currentPhaseState += phaseIncrement;

    // wrap the phase
    if (currentPhaseState > juce::MathConstants<double>::twoPi)
        currentPhaseState -= juce::MathConstants<double>::twoPi;
