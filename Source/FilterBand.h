...
struct FilterBand : public AudioProcessorParameter::Listener
{
    ...
    FilterBand(AudioProcessorType& processor, std::string bandName, float defaultFreq, int defaultType, double& fs) : samplerate(fs)
    {
        bandName.erase(std::find(bandName.begin(), bandName.end(), ' '));
       // Add the parameters. We get the name as an argument. Make a id tag from it by removing whitespaces and pushing to lower case.
        ...
        // Create parameters
        freqParam = new AudioParameterFloat (bandName + "freq", bandName + " Freq", 20, 20000, defaultFreq);
